...
class MCTS_fd(MCTS):
    ...
    def __init__(self,game,nnet,args):
        super().__init__(game,nnet,args)
        self.ivs = {} #immediate value [state]
        self.curr_root_npieces = 0
        self.curr_depthsa = {} # during the current getActionProb, the relative depth of each s,a pair
    
    def getActionProb(self, canonicalBoard, temp=1, dir_alpha=0,epsilon=0):
        self.curr_root_npieces = (canonicalBoard!=0).sum()
        prob = super().getActionProb(canonicalBoard, temp=temp,dir_alpha=dir_alpha, epsilon=epsilon)
        return prob
    
    def search(self, canonicalBoard):
        s = self.game.stringRepresentation(canonicalBoard)
        # s = np.array_str(canonicalBoard)
        npieces = np.sum(canonicalBoard!=0)
        ...
        if (npieces - self.curr_root_npieces) == self.args.max_depth:
            self.Ns[s]+=1
            return -self.ivs[s]
...
class MCTS_swap(MCTS):
    '''
    MCTS swap value head
    '''
    def __init__(self,game,nnet,nnet_swap,swap,args):
        super().__init__(game,nnet,args)
        self.nnet_swap = nnet_swap
        self.P_unused={}
        self.swap = swap

    def getActionProb(self, canonicalBoard, temp=1, dir_alpha=0,epsilon=0):
        probs = super().getActionProb(canonicalBoard, temp=temp,dir_alpha=dir_alpha, epsilon=epsilon)
        return probs
    ...
    def search(self, canonicalBoard):
        ...
        if s not in self.Ps:
            # leaf node
            if self.swap == 'v':
                self.Ps[s], v_unused= self.nnet.predict(canonicalBoard) # (boardsize,) scalar
                self.P_unused[s], v = self.nnet_swap.predict(canonicalBoard) # (boardsize,) scalar
            if self.swap == 'p':
                self.P_unused[s], v = self.nnet.predict(canonicalBoard) # (boardsize,) scalar
                self.Ps[s],v_unused = self.nnet_swap.predict(canonicalBoard) # (boardsize,) scalar
