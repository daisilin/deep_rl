...
from mcts import MCTS, MCTS_color,MCTS_fd,MCTS_swap
...
TOURNAMENT_NAME = 'tournament_1_extension_1'
...
def get_player(game,one_info,**kwargs):
    ...
    if one_info.value_func_type  == 'nn' or one_info.tree_type=='mcts': # both situation need the nn
        ...
        #[DL] if there is v or p swapping
        if one_info.swap == 'v' or one_info.swap == 'p': 
            load_swap_folder_file = one_info.nn_swap_location
            folder_swap = folder='/'.join(load_swap_folder_file.split('/')[:-1])
            file_swap =load_swap_folder_file.split('/')[-1]
            if one_info.n_res is None:    
                nnet_swap = NNetWrapper(game)
            else: # n_res not None
                args_swap = pickle.load(open(os.path.join(folder_swap,'args.p'),'rb'))
                if one_info.color: 
                    args['track_color']=True
                    othello_resnet = OthelloNNet_resnet(game,args_swap,return_compiled=True)
                    nnet_swap = NNetWrapper_color(game,args=args_swap,nnet=othello_resnet) 
                else:
                    othello_resnet = OthelloNNet_resnet(game,args_swap,return_compiled=True)
                    nnet_swap = NNetWrapper(game,args=args_swap,nnet=othello_resnet)
            nnet_swap.load_checkpoint(folder_swap, file_swap)
    if one_info.value_func_type=='nn':
        if one_info.swap == 'v' or one_info.swap == 'p':
            val_func = nnet
            value_func_swap = nnet_swap
        else:
            val_func = nnet
            
    elif one_info.value_func_type =='cog': #NB need to be changed; player_info should have the w and C
        w = [0.01,0.4,0.4,11,100]#[0.01,0.2,0.05,2,100]
        C = 0.1
        args = [w,C]
...
#[DL] modify to include depth manipulated agents
        if one_info.fix_depth == True:
            n_mcts = one_info.n_mcts
            cpuct = one_info.cpuct
            # args = MCTS_ARGS   
            args = dotdict({
            ...
            args['max_depth'] = one_info.max_depth
            tree = MCTS_fd(game,val_func,args)
        if one_info.swap == 'v' or one_info.swap == 'p': 
            args = dotdict({
            'numMCTSSims': n_mcts,
            'cpuct': cpuct,
            ...
            tree = MCTS_swap(game,val_func,value_func_swap,one_info.swap,args) #MCTS_swap
...
def get_nnet(game,one_info,**kwargs):
    '''
    one_info: a row of all_players/participants, as pd.series 
    '''
    n_available_actions = game.getActionSize()
